<script lang="ts">
	...
	import type { WorkflowTaskState } from '$lib/services/workflow-tasks';
...
<div class="app-shell grid h-dvh overflow-hidden bg-background">
	<header class="border-b border-border bg-surface shadow-lg shadow-black/40" style="grid-area: header">
		<Header {locale} {monthStatuses} activeYear={year} activeMonth={month} />
	</header>

	<LeftNav {locale} {workflowTasks} />
